...
- `python:S5713` in `app/icloud/icloud_service.py`: simplify `_decode_base64_filename` exception tuples by removing `binascii.Error`. In Python 3.11, `binascii.Error` subclasses `ValueError`, so catching both is redundant.
...
- `python:S5713` is valid on the supported Python version because `binascii.Error` inherits from `ValueError`.
...
1. Update `app/icloud/icloud_service.py`.
   ...
   - Remove `binascii.Error` from both exception tuples in `_decode_base64_filename`.
...
- If frontend tests still fail because of the known login test drift, record that as pre-existing and verify the `settings.js` / `main.js` cleanup manually or with focused tests once the drift is resolved.
